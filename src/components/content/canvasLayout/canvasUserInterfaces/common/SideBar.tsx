...
const SideBar: FC = () => {
  ...
  const handleClick = useCallback(
    (mapType: MapTypeEnum) => (e: React.MouseEvent<HTMLDivElement>) => {
      e.stopPropagation();
      setCurrentMyRoom(mapType === MapTypeEnum.GROUND ? me : undefined);
  ...
  return (
    <>
      <SideBarWrapper className={isDropDownOpen ? "opened" : "closed"}>
        <div onClick={handleClick(MapTypeEnum.GROUND)}>
          <SportsCricketIcon />
          <span>놀이터로 가기</span>
        </div>
        <div onClick={handleClick(MapTypeEnum.MYROOM)}>
          <HomeIcon />
          <span>내 방으로 가기</span>
        </div>
        <div onClick={handleClick(MapTypeEnum.GAMEROOM)}>
          <SportsEsportIcon />
          <span>게임 방으로 가기</span>
        </div>
      </SideBarWrapper>
